...
class Analysis:
    ...
    def var(self, headers, rows=[]):
        ...
        '''
        arr = self.data.select_data(headers, rows)
        top = arr - self.mean(headers, rows)
        return np.sum(np.multiply(top, top), axis = 0) / (len(arr) - 1)
        pass
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        '''
        #select data
        number = len(data_vars)
        arr = self.data.select_data(data_vars)

        # return fig and axes
        fig, axes = plt.subplots(nrows = number, ncols = number, sharex = 'col', sharey = 'row', figsize = fig_sz)
        fig.suptitle(title)

        for i in range(number):
            for j in range(number):

                axes[i, j].scatter(arr[:,j], arr[:,i])
                if j == 0:
                    # y axis of the first column
                    axes[i,j].set_ylabel(data_vars[i])
                if i == (number - 1):
                    # x axis of the last row labeled with the variable
